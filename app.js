...
async function run() {
  const uri =
    "mongodb+srv://aromatticindia:"+process.env.mongoDbPassword+"@aromattic.lyyczns.mongodb.net/?retryWrites=true&w=majority";

  const client = new MongoClient(uri);
  await client.connect();
  const dbName = "aromatticDatabase";
  ...
  try {
    const insertManyResult = await collection.insert(users);
